...
const components: { title: string; href: string; items?: {title:string,href:string}[] }[] = [
  {
    title: "Modas",
    href: "/s/moda",
    items:[
...
export function MenuDesktop({ initialUser }: MenuDesktopProps) {
  ...
  return (
    <>
      <NavigationMenu
        className="hidden relative lg:block w-full max-w-full mx-auto "
        viewport={isMobile}
      >
        <NavigationMenuList className="py-3 border-none w-full mx-auto sm:px-5 lg:px-14">
          <div className="font-semibold cursor-pointer">
            <Link href={"/"} className="font-title uppercase font-black">Rem/store</Link>
          </div>
...
const ListItem = React.forwardRef<HTMLAnchorElement, AnchorProps & { title: string, children: React.ReactNode }>(({ className, title, children, ...props }, ref) => {
  return (
      <li>
